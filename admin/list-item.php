<?php
	...
	$author_id = (int)@$filter['author_id'];
	
	//更新時
	if($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		$filter['category_id'] = (int)@$_POST['category_id'];
		$filter['author_id'] = (int)@$_POST['author_id'];
	...
	foreach($item_all as &$v)
	{
		...
		$v['author_name'] = htmlspecialchars($v['author_name'], ENT_QUOTES, 'UTF-8');
		$v['name'] = htmlspecialchars($v['name'], ENT_QUOTES, 'UTF-8');
		$v['item_authorname'] = htmlspecialchars($v['item_authorname'], ENT_QUOTES, 'UTF-8');
		unset($v);
	}
...
<td><?php echo $v['item_authorname']; ?></td>
