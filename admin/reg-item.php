<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST")
	{
		$data = array(
			...
			'item_authorname' => @$_POST['item_authorname'],
			'name'        => @$_POST['name'],
	...
	$item['item_authorname'] = htmlspecialchars(@$item['item_authorname'], ENT_QUOTES, 'UTF-8');
...
<!--
<form method="post" action="regtes.php?id=<?php echo $item_id; ?>" enctype="multipart/form-data" id="frmReg">
-->
...
<th>ライター名</th>
<td>
<select name="author_id" id="auth_id">
<option value="">全て</option>
<?php foreach($author_all as $v): ?>
<option value="<?php echo $v['author_id'];?>" <?php if(@$item['author_id']==$v['author_id'])echo 'selected="selected"'; ?>><?php echo $v['author_name'];?></option>
<?php endforeach;?>
</select>
<input type="hidden" name="item_authorname" id="auth_name" value="" />
...
<script> 
$(function($) {
	$authdata = $('#auth_id option:selected').text();
	$("#auth_name").val($authdata);

    $("select#auth_id").change(function() {
    	$authdata = $('#auth_id option:selected').text();
		$("#auth_name").val($authdata);
	});
});
...
<th>記事本文<br />[about]</th>
<td>
<textarea class="S07" name="about"><?php echo @$item['about']; ?></textarea>
<pre>
主な使用タグ
＜h2＞サブタイトル(大)＜/h2＞
＜h3＞サブタイトル(小)＜/h3＞
