<?php
	...
	class ExperienceModel extends BaseModel
	{
		...
		{
			parent::set_table_name('experience');
		}
		
		public function getReviewAllCount()
		{
			$pdo = PdoInterface::getInstance();
			$pdo->query("SELECT COUNT(*) FROM experience");
		...
		public function getReviewAllCountByFilter($category_id)
		{
			...
			$pdo = PdoInterface::getInstance();
			$pdo->query("SELECT COUNT(*) FROM experience LEFT JOIN item ON experience.item_id=item.item_id WHERE item.category_id=?", array($category_id));
			return $pdo->fetchColumn();
		}
		public function getReviewAllCountByItemId($item_id)
		{
			$pdo = PdoInterface::getInstance();
			$pdo->query("SELECT COUNT(*) FROM experience WHERE item_id=?", array($item_id));
			return $pdo->fetchColumn();
		}
		...
		{
			$pdo = PdoInterface::getInstance();
			$pdo->query("SELECT COUNT(*) FROM experience WHERE item_id=? AND disp=1", array($item_id));
			return $pdo->fetchColumn();
		}
		...
		public function getReviewAll($p, $page_count)
		{
			...
			$pdo->query("SELECT experience.*, 
					...
					FROM experience
					LEFT JOIN item     ON experience.item_id = item.item_id
					LEFT JOIN category ON experience.category_id = category.category_id
					LEFT JOIN division ON experience.division_id = division.division_id
					ORDER BY experience.archive_id DESC {$limit}");
		...
		public function getReviewAllByFilter($category_id, $p, $page_count)
		{
			...
			$pdo->query("SELECT experience.*,
					...
					ORDER BY experience.archive_id DESC {$limit}", array($category_id));
		...
		public function getReview($archive_id)
		{
		...
		public function getAttentionData($archive_id)
		{
			...
			$pdo->query("SELECT experience.*,
					...
					LEFT JOIN category ON experience.category_id = category.category_id
					LEFT JOIN division ON experience.division_id = division.division_id
					WHERE experience.archive_id=? ", array($archive_id));
		...
		{
			$pdo = PdoInterface::getInstance();
			$pdo->query("SELECT * FROM experience WHERE (item_id=? OR item_id=0) AND disp=1 ORDER BY update_time DESC", array($item_id));
		...
		{
			$pdo = PdoInterface::getInstance();
			$pdo->query("SELECT * FROM experience WHERE category_id=? AND (item_id=? OR item_id=0) AND disp=1 ORDER BY update_time DESC", array($category_id, $item_id));
		...
		public function getItemReview($item_data)
		{
			...
			//会社名を含めたクチコミ一覧を取得
			$pdo->query("SELECT * FROM experience WHERE disp=1 ORDER BY archive_id DESC");
